...
function update_player_skills(bonus_effect,bonus_amount)
	player:remove_effect(efftype_id(bonus_effect))
    if bonus_amount > 0 then
		player:add_effect((efftype_id(bonus_effect)), 1, "num_bp", true, bonus_amount)
	end
end

function calc_old_bonus(stat)
    return player:get_effect_int(efftype_id(stat), "num_bp")
end
